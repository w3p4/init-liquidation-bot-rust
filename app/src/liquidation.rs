...
>(
    ...
    let _ = liq_bot
        .flashLiquidateReturnNative(
            *pos_id,
            repay_pool.parse::<Address>()?, // pool to borrow
            router_1.parse::<Address>()?,   // swap borrow to repay
            repay_pool.parse::<Address>()?, // pool to repay
            router_2.parse::<Address>()?,   // swap coll to repay borrow
            coll_pool.parse::<Address>()?,  // coll pool
            router_3.parse::<Address>()?,   // swap coll to return native
            parse_ether(profit).unwrap(),   // native minimum amount
        )
        .send()
        .await?;
    Ok(())
